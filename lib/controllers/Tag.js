...
exports.add = function(req,res) {
    ...
    newTag.save(function(err) {
        ...
        _.each(UserConnections, function(UserConnection) {
            UserConnection.socket.emit("new:tag", newTag); 
        });
...
exports.update = function(req,res) {
    Tag.findById(req.params.id, function (err, tag) {
        ...
        tag.save(function (err) {
            ...
            var UserConnections = _.filter(connections, function(connection){ return connection.user._id === req.user.id;});
            _.each(UserConnections, function(UserConnection) {
                UserConnection.socket.emit("update:tag", tag); 
            });
            res.send(tag);
        });
    });
...
exports.delete = function(req,res) {
    ...
    Tag.remove({_id: id}, function(err, resp){
        if(err) return res.json(400, err);
        var UserConnections = _.filter(connections, function(connection){ return connection.user._id === req.user.id;});
        _.each(UserConnections, function(UserConnection) {
            UserConnection.socket.emit("delete:tag", {_id: id}); 
        });
        res.send('success');
    });
};
